                          ...
                          <tbody>
                            @foreach ($mapAkun as $item)
                              <tr>
                                ...
                                <td>{{ $item->kode_akun_1 }}</td>
                                <td>{{ $item->kode_akun_2 }}</td>
                                <td>{{ $item->kode_akun_3 }}</td>
                                @if (Auth::user()->hasRole('admin'))
                                <td>
                                  <button class="btn btn-sm btn-warning btn-edit"
                                    ...
                                    data-kode-akun-1="{{ $item->kode_akun_1 ?? '' }}"
                                    data-kode-akun-2="{{ $item->kode_akun_2 ?? '' }}"
                                    data-kode-akun-3="{{ $item->kode_akun_3 ?? '' }}"
                                    >
...
<div class="modal fade" tabindex="-1" role="dialog" id="createModal">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="{{ route('admin.pemetaan_akun.store') }}" method="POST">
        @csrf
        <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <select name="kode_akun_1" class="form-control selectpicker" data-live-search="true" title="Pilih Kode">
            ...
            <div class="form-group">
              ...
              <select name="kode_akun_2" class="form-control selectpicker" data-live-search="true" title="Pilih Kode">
                @foreach ($allAkun as $item)
                  <option value="{{ $item->kode_akun }}">{{ $item->kode_akun }} - {{ $item->nama_akun }}</option>
                @endforeach
              </select>
            ...
            <div class="form-group">
              ...
              <select name="kode_akun_3" class="form-control selectpicker" data-live-search="true" title="Pilih Kode">
                @foreach ($allAkun as $item)
                  <option value="{{ $item->kode_akun }}">{{ $item->kode_akun }} - {{ $item->nama_akun }}</option>
                @endforeach
              </select>
...
<div class="modal fade" tabindex="-1" role="dialog" id="editModal">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="{{ route('admin.pemetaan_akun.update') }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label>Kode Pemetaan Akun 1</label>
            <select id="edit-kode-akun-1" name="kode_akun_1" class="form-control selectpicker" title="Pilih Kode">
              @foreach ($allAkun as $item)
                <option value="{{ $item->kode_akun }}">{{ $item->kode_akun }} - {{ $item->nama_akun }}</option>
              @endforeach
            </select>
          ...
          <div class="form-group">
            <label>Kode Pemetaan Akun 2</label>
            <select id="edit-kode-akun-2" name="kode_akun_2" class="form-control selectpicker" title="Pilih Kode">
              @foreach ($allAkun as $item)
                <option value="{{ $item->kode_akun }}">{{ $item->kode_akun }} - {{ $item->nama_akun }}</option>
              @endforeach
            </select>
          ...
          <div class="form-group">
            <label>Kode Pemetaan Akun 3</label>
            <select id="edit-kode-akun-3" name="kode_akun_3" class="form-control selectpicker" title="Pilih Kode">
              @foreach ($allAkun as $item)
                <option value="{{ $item->kode_akun }}">{{ $item->kode_akun }} - {{ $item->nama_akun }}</option>
              @endforeach
            </select>
...
{{-- form delete --}}
<form id="form-delete" action="{{ route('admin.pemetaan_akun.destroy') }}" class="d-none" method="POST">
  @csrf
  @method('DELETE')
...
<script>
  $(document).ready(function () {
    ...
    $('.btn-edit').click(function () {
      ...
      const kodeAkun1 = $(this).attr('data-kode-akun-1');
      const kodeAkun2 = $(this).attr('data-kode-akun-2');
      const kodeAkun3 = $(this).attr('data-kode-akun-3');

      // auto select options
      $('#edit-kode-akun.selectpicker').val(kodeAkun);
      $('#edit-kode-akun-1.selectpicker').val(kodeAkun1);
      $('#edit-kode-akun-2.selectpicker').val(kodeAkun2);
      $('#edit-kode-akun-3.selectpicker').val(kodeAkun3);
