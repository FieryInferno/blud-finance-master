                    ...
                    <div class="form-group">
                      <div class="row">
                        <div class="col">
                          ...
                          <div class="row">
                            <div class="col-md-6">
                              <input type="text" class="form-control" name="nomor" placeholder="(Auto)" value="{{ $saldoAwal->nomor }}" readonly>
                            </div>
                              ...
                              <select name="unit_kerja" id="unit_kerja" class="form-control" {{ (auth()->user()->hasRole('Puskesmas') ? 'readonly' : '') }}>
                                  ...
                                  @foreach ($unitKerja as $item)
                                      <option value="{{ $item->kode }}"
                                        {{ ($saldoAwal->kode_unit_kerja == $item->kode ? 'selected' : '') }}
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col">
                                ...
                                <input type="text" class="form-control" name="keterangan" value="{{ $saldoAwal->keterangan }}">
                            </div>
                        </div>
                    </div>
                  </div>
                            ...
                            <div class="row">
                              <div class="col-md-12">
                                ...
                                <table class="table" id="table-saldo-awal">
                                  ...
                                  <tbody>
                                    @foreach ($saldoAwal->saldoAwalRincian as $item)
                                        <tr>
                                          <td><button type="button" class="btn btn-remove btn-sm btn-danger"><i class="fas fa-minus"></i></button></td>
                                          <td><input type="text" class="form-control" name="kode_akun[]" value="{{ $item->akun->kode_akun }}" readonly></td>
                                          <td><input type="text" class="form-control" name="" value="{{ $item->akun->nama_akun }}" readonly></td>
                                          <td><input type="text" class="form-control money" name="debet[]" value="{{ format_report($item->debet) }}"></td>
                                          <td><input type="text" class="form-control money" name="kredit[]" value="{{ format_report($item->kredit) }}"></td>
                                        </tr>
                                    @endforeach
                                  </tbody>
                                </table>
              ...
              <tbody>
                @foreach ($akunLo as $item)
                  <tr class="{{ $item->is_parent ? 'table-primary' : '' }}" data-kode-akun="{{ $item->kode_akun }}" data-nama-akun="{{ $item->nama_akun }}">
                    <td>
                      @if (! $item->is_parent)
                          <input type="checkbox" {{ in_array($item->id, $akunId) ? 'checked' : '' }}>
                      @endif
                    </td>
...
@section('js')
  ...
  <script>
      ...
      $(document).ready(function () {
        ...
        $(".date").datepicker('update', '{{ $saldoAwal->tanggal }}');
        ...
        $.ajax({
          type: "PUT",
          url: "{{ route('admin.saldo_neraca.update', $saldoAwal->id ) }}",
          data: formData,
          beforeSend:function() {
