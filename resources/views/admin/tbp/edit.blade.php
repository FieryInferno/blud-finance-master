...
<div class="main-content">
  <section class="section">
      ...
      <form action="{{ route('admin.tbp.update', $tbp->id) }}" method="POST" id="form-tbp">
      @csrf
      @method('PUT')
                    ...
                    <div class="form-group">
                      <div class="row">
                        <div class="col">
                          ...
                          <input type="text" class="form-control" name="nomor" value="{{ $tbp->nomorfix }}" readonly>
                        </div>
                      </div>
                    </div>
                     ...
                     <div class="form-group">
                      <div class="row">
                        <div class="col">
                          ...
                          <input type="text" class="form-control" name="tanggal" id="date" value="{{ $tbp->tanggal }}">
                        </div>
                      </div>
                    </div>
                                ...
                                <select name="unit_kerja" id="unit_kerja" class="form-control" {{ (auth()->user()->hasRole('Puskesmas') ? 'readonly' : '') }}>
                                    ...
                                    @foreach ($unitKerja as $item)
                                        <option value="{{ $item->kode }}"
                                          ...
                                          @else
                                            {{ ($tbp->kode_unit_kerja == $item->kode ? 'selected' : '') }}
                                          @endif
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col">
                                ...
                                <input type="text" class="form-control" name="keterangan" value="{{ $tbp->keterangan }}">
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                                ...
                                <select name="rekening_bendahara" id="rekening_bendahara" class="form-control">
                                  @foreach ($rekeningBendahara as $item)
                                      <option value="{{ $item->id }}"
                                         @if ($item->id == $tbp->rekening_bendahara_id) selected="selected" @endif>{{ $item->kode_akun }} {{ $item->nama_akun_bendahara }}</option>
                                  @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="tab-pane active show" id="rincian_anggaran" role="tabpanel" aria-labelledby="home-tab">
                                ...
                                <div class="col">
                                    ...
                                    <table class="table table-rba">
                                      ...
                                      <tbody>
                                        @foreach ($tbp->rincianTbp as $item)
                                          <tr>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="kepala_skpd" class="form-control" id="kepala_skpd">
                                             @foreach ($pejabatUnit as $item)
                                                <option value="{{ $item->id }}"
                                                  @if ($item->id == $tbp->kepala_skpd) selected="selected" @endif>
                                                  {{ $item->nama_pejabat }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col">
                                    <div class="form-group">
                                        ...
                                        <select name="bendahara_penerima" class="form-control" id="bendahara_penerima">
                                            @foreach ($pejabatUnit as $item)
                                                <option value="{{ $item->id }}"
                                                  @if ($item->id == $tbp->bendahara_penerima) selected="selected" @endif>
                                                  {{ $item->nama_pejabat }}
                                                </option>
                                            @endforeach
                                        </select>
...
@section('js')
  ...
  <script>
      ...
      $(document).ready(function () {
        ...
        $("#date").datepicker('update', '{{ $tbp->tanggal }}');
        ...
        $.ajax({
          type: "PUT",
          url: "{{ route('admin.tbp.update', $tbp->id) }}",
          data: formData,
          beforeSend:function() {
