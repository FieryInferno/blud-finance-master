...
<div class="main-content">
  <section class="section">
      <form action="{{ route('admin.bku.update', $bku->id) }}" method="POST" id="form-tbp">
      @method('PUT')
      @csrf
                    ...
                    <div class="form-group">
                      <div class="row">
                        <div class="col">
                          ...
                          <div class="row">
                            <div class="col-md-9">
                              <input type="text" class="form-control" name="nomor" value="{{ $bku->nomor }}" readonly>
                            </div>
                            <div class="col-md-3">
                    ...
                    <div class="form-group">
                      <div class="row">
                        <div class="col">
                          ...
                          <input type="text" class="form-control date" name="tanggal_bku" value="{{ $bku->tanggal }}">
                        </div>
                      </div>
                    </div>
                                ...
                                <select name="unit_kerja" id="unit_kerja" class="form-control" {{ (auth()->user()->hasRole('Puskesmas') ? 'readonly' : '') }}>
                                    ...
                                    @foreach ($unitKerja as $item)
                                        <option value="{{ $item->kode }}"
                                          {{ ($bku->kode_unit_kerja == $item->kode) ? 'selected' : '' }}>{{ $item->kode }} - {{ $item->nama_unit }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col">
                                ...
                                <input type="text" class="form-control" name="keterangan" value="{{ $bku->keterangan }}">
                            </div>
                        </div>
                    </div>
                                    ...
                                    <table class="table table-rba">
                                      ...
                                      <tbody>
                                        @foreach ($bku->bkuRincian as $item)
                                          <tr>
                                            <td>
                                              <button type="button" class="btn btn-remove btn-sm btn-danger"><i class="fas fa-minus"></i></button><input type="hidden" name="sts_id[]" value="{{ $item->sts_id }}">
                                            </td>
                                            <td>
                                              <input type="text" name="no_aktivitas[]" class="form-control" value="{{ $item->no_aktivitas }}" readonly><input type="hidden" name="sp2d_id[]" value="{{ $item->sp2d_id }}">
                                            </td>
                                            <td>
                                              <input type="text" name="tipe[]" class="form-control" value="{{ $item->tipe }}" readonly><input type="hidden" name="setor_pajak_id[]" value="{{ $item->setor_pajak_pajak_id }}">
                                            </td>
                                            <td>
                                              <input type="text" name="tanggal[]" class="form-control" value="{{ $item->tanggal }}" readonly><input type="hidden" name="kontrapos_id[]" value="{{ $item->kontrapos_id }}">
                                            </td>
                                            <td>
                                              <input type="text" name="penerimaan[]" class="form-control money" value="{{ $item->setorPajak ? format_report(0) : format_report($item->penerimaan)  }}" readonly>
                                            </td>
                                            <td>
                                              <input type="text" name="pengeluaran[]" class="form-control money" value="{{ (int) $item->pengeluaran }}" readonly>
                                            </td>
                                            <td>
                                              <input type="text" name="kode_unit_kerja[]" class="form-control" value="{{ $item->kode_unit_kerja }}" readonly>
                                            </td>
                                            <td>
                                              <input type="text" class="form-control" value="{{ $item->unitKerja->nama_unit }}" readonly>
                                            </td>
                                          </tr>
                                        @endforeach
                                      </tbody>
      ...
      <div class="modal-header">
        ...
        <div class="modal-body">
            <table class="table table-sp2d"  style="width:100%">
              ...
              <tbody>
                {{-- <tr data-id="1" data-nomor="101010" data-tanggal="2020-01-23"
                  data-nominal="200000" data-keterangan="Coba coba" data-unit-kerja="02" data-nama-unit-kerja="Halo">
                  <td><input type="checkbox" name="rekening" value="1"></td>
                  <td>1010101</td>
                  <td>2020-01-20</td>
                  <td>Dor</td>
                  <td>AA</td>
...
@section('js')
    ...
    <script>
        ...
        $('.date').datepicker('update', '{{ $bku->tanggal }}');
      ...
      $('#form-tbp').on('submit', function (e) {
        ...
        $.ajax({
          type: "PUT",
          url: "{{ route('admin.bku.update', $bku->id) }}",
          data: formData,
          beforeSend:function() {
      ...
      function getSetorPajak(){
        $.ajax({
              type: "GET",
              url: "{{ route('admin.setor_pajak.data') }}",
              data: "kode_unit_kerja="+$("#unit_kerja").val(),
              success:function(response) {
                let data = response.data
