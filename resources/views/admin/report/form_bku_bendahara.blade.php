...
<body>
    <div class="container">
        ...
        @php
            $penerimaanPindah = 0;
            $pengeluaranPindah = 0;
            $saldoSebelumnya = $penerimaanPreviousMonth - $pengeluaranPreviousMonth;
            $counter = 0;
        @endphp
        ...
        <table class="content">
            ...
            <tr>
                ...
                <td class="text-right borderAll">({{ format_report($saldoSebelumnya) }})</td>
            </tr>
            ...
            @foreach ($group as $index => $item)
                ...
                <tr>
                    ...
                    <td class="text-left borderAll">{{ $item->keterangan }}</td>
                    <td class="text-left borderAll"></td>
                ...
                @if ($item->bkuRincian)
                    <tr>
                        ...
                        <td class="text-left borderAll">{{ report_date($item->bkuRincian->bku->tanggal) }}</td>
                        <td class="text-left borderAll">{{ $item->bkuRincian->bku->keterangan }}</td>
                        <td class="text-left borderAll"></td>
                        <td class="text-right borderAll">0,00</td>
                        <td class="text-right borderAll">{{ format_idr($item->nominal) }}</td>
                    </tr>
            ...
            @else
                <tr>
                    ...
                    <td class="text-right borderAll">{{ format_report($penerimaanFirstMonth) }}</td>
                    <td class="text-right borderAll">{{ format_report($pengeluaranFirstMonth) }}</td>
                </tr>

                <tr>
                    <td class="text-left borderAll" colspan="4">Jumlah bulan lalu</td>
                    <td class="text-right borderAll">{{ format_report($penerimaanPreviousMonth) }}</td>
                    <td class="text-right borderAll">{{ format_report($pengeluaranPreviousMonth) }}</td>
                </tr>
                <tr>
                    <td class="text-left borderAll" colspan="4">Jumlah s/d bulan {{ bulan($lastMonth) }}</td>
                    <td class="text-right borderAll">{{ format_report($penerimaanFirstMonth + $penerimaanPreviousMonth) }}</td>
                    <td class="text-right borderAll">{{ format_report($pengeluaranFirstMonth + $pengeluaranPreviousMonth) }}</td>
                </tr>

                <tr>
                    <td class="text-left borderAll" colspan="4">Sisa Kas</td>
                    <td class="text-right borderAll" colspan="2">{{ format_report(($penerimaanFirstMonth + $penerimaanPreviousMonth)- ($pengeluaranFirstMonth + $pengeluaranPreviousMonth)) }}</td>
                </tr>
            @endif
        </table>
        @endforeach
        ...
        <div class="footer">
            ...
            <div class="col-print-6 text-center">
                ...
                <u>{{ $bendaharaPenerimaan ? $bendaharaPenerimaan->nama_pejabat : '' }}</u> <br/>
                NIP. {{ $bendaharaPenerimaan ? $bendaharaPenerimaan->nip : '' }}
            </div>
        </div>
    </div>
</body>
