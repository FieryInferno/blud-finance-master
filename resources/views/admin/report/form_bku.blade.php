            ...
            <tbody>
                ...
                @foreach ($bku->bkuRincian as $item)
                    <tr>
                        ...
                        @elseif ($item->sp2d)
                            <td class="borderLeft">{{ $item->tipe }} No {{ $item->no_aktivitas }} {{ $item->tipe }} {{ $item->sp2d->keterangan }} {{ $item->sp2d->unitKerja->nama_unit }} {{ format_idr($item->pengeluaran) }}</td>
                        @elseif ($item->setorPajak)
                            <td class="borderLeft">{{ $item->tipe }} No {{ $item->no_aktivitas }} {{ $item->tipe }} {{ $item->setorPajak->pajak->nama_pajak }} {{ $bku->unitKerja->nama_unit }} {{ format_idr($item->pengeluaran) }}</td>
                        @elseif ($item->kontrapos)
                            <td class="borderLeft">{{ $item->tipe }} No {{ $item->no_aktivitas }} {{ $item->tipe }} {{ $item->kontrapos->keterangan }} {{ $bku->unitKerja->nama_unit }} {{ format_idr($item->penerimaan) }}</td>
                        @else
                            <td class="borderLeft"></td>
                        @endif
                        <td class="borderLeft text-right">{{ $item->setorPajak ? format_idr(0) : format_idr($item->penerimaan) }}</td>
                        <td class="borderLeft text-right">{{ format_idr($item->pengeluaran) }}</td>
                    </tr>
                @endforeach

                <tr>
                    <td colspan="2" class="font-weight-bold text-center">Jumlah tanggal {{ report_date($bku->tanggal) }}</td>
                    <td class="font-weight-bold text-right">{{ format_idr($totalPenerimaan) }}</td>
                    <td class="font-weight-bold text-right">{{ format_idr($totalPengeluaran) }}</td>
                </tr>

                <tr>
                    <td colspan="2" class="font-weight-bold text-center">Jumlah sampai dengan tanggal {{ report_date($bku->yesterday) }}</td>
                    <td class="font-weight-bold text-right">{{ format_idr($totalPenerimaanYesterday+$saldoAwal-$totalPengeluaranYesterday) }}</td>
                    <td class="font-weight-bold text-right">{{ format_idr(0) }}</td>
                </tr>

                <tr>
                    <td colspan="2" class="font-weight-bold text-center">Jumlah sampai dengan tanggal {{ report_date($bku->tanggal) }}</td>
                    <td class="font-weight-bold text-right">{{ format_idr($totalPenerimaanYesterday+$saldoAwal-$totalPengeluaranYesterday+$totalPenerimaan) }}</td>
                    <td class="font-weight-bold text-right">{{ format_idr($totalPengeluaran) }}</td>
                </tr>

                <tr>
                    <td colspan="2" class="font-weight-bold text-center">Saldo hari ini tanggal {{ report_date($bku->tanggal) }}</td>
                    <td class="font-weight-bold text-right"></td>
                    <td class="font-weight-bold text-right">{{ format_idr($totalPenerimaanYesterday+$saldoAwal-$totalPengeluaranYesterday+$totalPenerimaan-$totalPengeluaran) }}</td>
                </tr>

                <tr>
                    <td colspan="4" class="font-weight-bold text-left">Sisa dengan huruf: {{  strtoupper(terbilang($totalPenerimaanYesterday+$saldoAwal-$totalPengeluaranYesterday+$totalPenerimaan-$totalPengeluaran)) }}</td>
                </tr>
            </tbody>
        </table>
        <div class="clearfix"></div>
